export class Timeline {
  ...
  init(){
    setInterval( () => this.drawPathHighlight(true), 50);
  ...
  drawPathHighlight(rotate) {   
    ...
    //ring
    this.ctx.beginPath();
    this.ctx.arc(this._currentPoint.x, this._currentPoint.y, 30, 0, 2 * Math.PI);
    this.ctx.stroke();

    //partial top ring
    this.ctx.beginPath();
    this.ctx.arc(this._currentPoint.x, this._currentPoint.y, 35, 1.5 * Math.PI, 1.8 * Math.PI);
    this.ctx.stroke();

    //partial bottom ring
    this.ctx.beginPath();
    this.ctx.arc(this._currentPoint.x, this._currentPoint.y, 35, .5 * Math.PI, .8 * Math.PI);
    this.ctx.stroke();

    //animaged inner ring
    //ring
    if (rotate) this.rotation >= 1 ? this.rotation = 0 : this.rotation += .05;
    ...
    this.ctx.arc(
        ...
        this._currentPoint.y, 
        23, 
        (this.rotation + .5) * 2* Math.PI, 
        (this.rotation + .75) * 2* Math.PI
        );
    this.ctx.stroke(); 
  }
  ...
  drawCurrentPositionIndicator() {
    ...
    this.ctx.lineTo(this._slideBounds.x + this._currentPosition, this._slideBounds.y + 7);
    this.ctx.lineTo(this._slideBounds.x + this._currentPosition, this._slideBounds.y + 1);
    this.ctx.lineTo(
